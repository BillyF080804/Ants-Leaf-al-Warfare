...
public class QueenAntSpawner : MonoBehaviour {
    ...
    private void Update() {
        if (targetObject != null) {
            ...
            if (Physics.Raycast(targetPos, Vector3.down, out RaycastHit ray, 35.0f, ~targetObject.GetComponent<QueenBaseAntScript>().GetQueenLayerMask())) {
    ...
    public void CheckQueenInValidPos() {
        if (Physics.OverlapSphere(targetObject.transform.position, turnManager.MinDistanceBetweenQueens).Where(x => x.CompareTag("Player")).Count() > 0) {
            ...
            targetObject.GetComponent<QueenBaseAntScript>().SetQueenInvalidPos();
        }
        else {
            QueenInValidPos = true;
            targetObject.GetComponent<QueenBaseAntScript>().SetQueenValidPos();
        }
    }
}